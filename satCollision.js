function workOutNewPoints(cx, cy, vx, vy, rotatedAngle){ //From a rotated object
    //cx,cy are the centre coordinates, vx,vy is the point to be measured against the center point
        ...
        let dx = vx - cx; //distance from center
        let dy = vy - cy;
...
function getRotatedSquareCoordinates(square){
    ...
    let bottomLeft;
    let bottomRight;
    if(square.currentAngle){
        console.log('ANGULAR')
        topLeft = workOutNewPoints(centerX, centerY, square.x, square.y, square.currentAngle);
        topRight = workOutNewPoints(centerX, centerY, square.x + square.width, square.y, square.currentAngle);
        bottomLeft = workOutNewPoints(centerX, centerY, square.x, square.y + square.height, square.currentAngle);
        bottomRight = workOutNewPoints(centerX, centerY, square.x + square.width, square.y + square.height, square.currentAngle);
    }
    else{
        topLeft = workOutNewPoints(centerX, centerY, square.x, square.y, 0);
        topRight = workOutNewPoints(centerX, centerY, square.x + square.width, square.y, 0);
        bottomLeft = workOutNewPoints(centerX, centerY, square.x, square.y + square.height, 0);
        bottomRight = workOutNewPoints(centerX, centerY, square.x + square.width, square.y + square.height, 0);
    }
